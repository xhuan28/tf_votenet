...
class SunrgbdDetectionVotesDataset(object):
    def __init__(self, split_set='train', batch_size=8, num_points=20000, augment=False):
        assert(num_points<=50000)
        self.data_path = os.path.join('/home/intel/workspace/votenet/sunrgbd/', 'sunrgbd_pc_bbox_votes_50k_v1_%s' %split_set)
        self.raw_data_path = '/home/intel/workspace/votenet/sunrgbd/sunrgbd_trainval'
...
#        return self._get_item(idx)
    ...
    def reset(self):
        self.idxs = np.arange(0, len(self.scan_names))
        if self.shuffle:
            np.random.shuffle(self.idxs)
        self.num_batches = (len(self.scan_names)+self.batch_size-1) // self.batch_size
        self.batch_idx = 0
    ...
    def next_batch(self):
        ...
        end_idx = min((self.batch_idx+1) * self.batch_size, len(self.scan_names))
        bsize = end_idx - start_idx
        batch_data_label = {}
        batch_data_label['point_clouds'] = np.zeros((bsize, self.num_points, self.num_channel()), dtype=np.float32)
        batch_data_label['center_label'] = np.zeros((bsize, MAX_NUM_OBJ, 3), dtype=np.float32)
        batch_data_label['heading_class_label'] = np.zeros((bsize, MAX_NUM_OBJ), dtype=np.int64)
        batch_data_label['heading_residual_label'] = np.zeros((bsize, MAX_NUM_OBJ), dtype=np.float32)
        batch_data_label['size_class_label'] = np.zeros((bsize, MAX_NUM_OBJ), dtype=np.int64)
        batch_data_label['size_residual_label'] = np.zeros((bsize, MAX_NUM_OBJ, 3), dtype=np.float32)
        batch_data_label['sem_cls_label'] = np.zeros((bsize, MAX_NUM_OBJ), dtype=np.int64)
        batch_data_label['box_label_mask'] = np.zeros((bsize, MAX_NUM_OBJ), dtype=np.float32)
        batch_data_label['vote_label'] = np.zeros((bsize, NUM_POINTS, 9), dtype=np.float32)
        batch_data_label['vote_label_mask'] = np.zeros((bsize, NUM_POINTS), dtype=np.int64)
        batch_data_label['scan_idx'] = np.zeros((bsize), dtype=np.int64)
        batch_data_label['max_gt_bboxes'] = np.zeros((bsize, MAX_NUM_OBJ, 8))

        for i in range(bsize):
            ret_dict = self.__getitem__(self.idxs[i+start_idx])
            for key in ret_dict:
                batch_data_label[key][i] = ret_dict[key]
        self.batch_idx += 1
...
if __name__=='__main__':
    d = SunrgbdDetectionVotesDataset(split_set='train', augment=True)
    print(len(d))
    print(d.num_batches)
